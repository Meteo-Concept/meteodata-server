/**
 * @file virtual_obs_computer_standalone.cpp
...
int main(int argc, char** argv)
{
	...
	try {
		...
		int retry = 0;
		for (auto it = virtualStations.cbegin() ; it != virtualStations.cend() ;) {
			const auto& station = *it;
			if (!userSelection.empty()) {
				if (userSelection.find(station.station) == userSelection.cend()) {
					++it;
					continue;
				}
			}

			std::cerr << "About to download for station " << station.station << std::endl;
			VirtualObsComputer computer{station, db};
