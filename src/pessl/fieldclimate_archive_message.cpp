...
Observation FieldClimateApiArchiveMessage::getObservation(const CassUuid& station) const
{
	...
	result.heatindex = {!isInvalid(_obs.temperature) && !isInvalid(_obs.humidity),
	...
	if (!isInvalid(_obs.temperature) && !isInvalid(_obs.windSpeed) && !isInvalid(_obs.humidity) &&
		...
		result.et = {true,
					 evapotranspiration(_obs.temperature, _obs.humidity, from_kph_to_mps(_obs.windSpeed), _obs.solarRad,
		...
		result.thswindex = {true, thsw_index(_obs.temperature, _obs.humidity, from_kph_to_mps(_obs.windSpeed),
											 _obs.solarRad)};
	...
	result.windchill = {!isInvalid(_obs.temperature) && !isInvalid(_obs.windSpeed),
	...
	if (!isInvalid(_obs.solarRad)) {
		bool ins = insolated(_obs.solarRad, _timeOffseter->getLatitude(), _timeOffseter->getLongitude(),
