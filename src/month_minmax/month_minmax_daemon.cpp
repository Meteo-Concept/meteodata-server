/**
 * @file month_minmax_daemon.cpp
 * @brief MonthMinmax recomputing program
...
#include "config.h"
#include "month_minmax/month_minmax_worker.h"
...
{
	MonthMinmaxWorker::Configuration serverConfig;
	bool daemonized;
	...
	config.add_options()
		...
		("pguser", po::value<std::string>(&serverConfig.pguser), "PostgreSQL database username")
		("pgpassword", po::value<std::string>(&serverConfig.pgpassword), "PostgreSQL database password")
		("pghost", po::value<std::string>(&serverConfig.pgaddress), "PostgreSQL database IP address or domain name")
		("stations-db-user", po::value<std::string>(&serverConfig.stationsDbUsername), "normals database username")
		("stations-db-password", po::value<std::string>(&serverConfig.stationsDbPassword), "normals database password")
		("stations-db-host", po::value<std::string>(&serverConfig.stationsDbAddress), "normals database IP address or domain name")
		("stations-db-database", po::value<std::string>(&serverConfig.stationsDbDatabase), "normals database IP address or domain name")
	...
	try {
		...
		MonthMinmaxWorker minmaxer{serverConfig, ioContext};
		minmaxer.start();
		...
		if (daemonized)
			sd_notifyf(0, "READY=1\n" "STATUS=MonthMinmax recomputing monitor started\n" "MAINPID=%d", getpid());
