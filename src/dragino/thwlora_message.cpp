/**
 * @file thwlora_message.cpp
 * @brief Implementation of the ThwloraMessage class
...
ThwloraMessage::ThwloraMessage(DbConnectionObservations& db):
	_db{db}
{}

void ThwloraMessage::ingest(const CassUuid& station, const std::string& payload, const date::sys_seconds& datetime)
{
...
Observation ThwloraMessage::getObservation(const CassUuid& station) const
{
	Observation obs;
	obs.station = station;
	...
	obs.min_windspeed = {!std::isnan(_obs.minWindSpeed), _obs.minWindSpeed};
	obs.winddir = {!std::isnan(_obs.windDir), int(std::round(_obs.windDir))};
	obs.voltage_battery = {!std::isnan(_obs.battery), _obs.battery};
	return obs;
}

json::object ThwloraMessage::getDecodedMessage() const
{
	return json::object{
		{ "model", "Thwlora_20251103" },
		{ "value", {
			...
			{ "min_wind_speed", _obs.minWindSpeed },
