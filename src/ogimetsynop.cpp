...
{
	double rh = std::expm1(((17.27 * dewPoint) / (23.77 + dewPoint)) - ((17.27 * temperature) / (23.7 + temperature)));
	return int((rh + 1.) * 100);
}
}
...
void OgimetSynop::populateDataPoint(const CassUuid station, CassStatement* const statement) const
{
	...
	if (_data._pressureAtSeaLevel)
		cass_statement_bind_float(statement, 3, *_data._pressureAtSeaLevel);
	/*************************************************************/
	// No absolute barometric pressure
	/*************************************************************/
	if (_data._pressureAtStation)
		cass_statement_bind_float(statement, 3, *_data._pressureAtStation);
	/*************************************************************/
	...
	// No soil moisture
	/*************************************************************/
	if (_data._meanWindSpeed)
		cass_statement_bind_float(statement, 40,
			_data._windSpeedUnit == SynopMessage::WindSpeedUnit::KNOTS ?
				*_data._meanWindSpeed * 1.85200 :
				*_data._meanWindSpeed * 3.6
		);
	/*************************************************************/
	if (_data._meanWindDirection)
		cass_statement_bind_int32(statement, 41, *_data._meanWindDirection * 10.);
	/*************************************************************/
	...
	if (_data._globalSolarRadiationLastHour)
		cass_statement_bind_int32(statement, 56, *_data._globalSolarRadiationLastHour);
	/*************************************************************/
	if (_data._dewPoint)
	...
	/*************************************************************/
	if (_data._meanTemperature && _data._meanWindSpeed)
	...
	&& _humidity && _data._globalSolarRadiationLastHour)
		cass_statement_bind_float(statement, 60,
			thsw_index(
				*_data._meanTemperature,
				*_humidity,
				*_data._meanWindSpeed,
				*_data._globalSolarRadiationLastHour
			)
		);
	else if (_data._meanTemperature && _data._meanWindSpeed
	 && _humidity && _data._globalSolarRadiationLastHour)
		cass_statement_bind_float(statement, 60,
			thsw_index(
				*_data._meanTemperature / 10.,
...
void OgimetSynop::populateV2DataPoint(const CassUuid station, CassStatement* const statement) const
{
	...
	/*************************************************************/
	if (_data._globalSolarRadiationLastHour)
		cass_statement_bind_int32(statement, 30, *_data._globalSolarRadiationLastHour);
	/*************************************************************/
	if (_data._meanTemperature && _data._meanWindSpeed
	&& _humidity && _data._globalSolarRadiationLastHour)
	...
	/*************************************************************/
	if (_data._meanWindDirection)
		cass_statement_bind_int32(statement, 34, *_data._meanWindDirection * 10);
	/*************************************************************/
	// No max wind speed
	/*************************************************************/
	if (_data._meanWindSpeed)
