/**
 * @file weatherlink_download_apiv2_ingester_standalone.cpp
	...
	try {
		...
		for (auto it = weatherlinkStations.cbegin() ; it != weatherlinkStations.cend() ; ++it) {
			...
			if (!std::get<1>(station)) {
				...
				try {
					downloader.ingestRealTime();
				} catch (std::runtime_error& e) {
					std::cerr << "Getting the data failed: " << e.what() << std::endl;
				}
			} else {
				std::cerr << "Not ingesting realtime data for station " << std::get<0>(station)
					  << ", we have access to archive" << std::endl;
			}
		}
