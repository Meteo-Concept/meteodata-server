...
namespace meteodata {
			...
			_db{db}
	{
		...
		for (auto&& s : mqttStations) {
			...
			if (std::regex_search(topic, match, userName)) {
				_userAndTimezoneByStation[std::get<0>(s)] = { match[1].str(), TimeOffseter::PredefinedTimezone(std::get<7>(s)) };
			}
		}
	}
	...
	bool VantagePro2HttpRequestHandler::getUuidAndCheckAccess(const Request& request, Response& response, CassUuid& uuid, const std::cmatch& url)
	{
	...
	void VantagePro2HttpRequestHandler::postArchivePage(const Request& request, Response& response, std::cmatch&& url) {
		...
		if (getUuidAndCheckAccess(request, response, uuid, url)) {
			...
			if (size % sizeof(VantagePro2ArchiveMessage::ArchiveDataPoint) != 0) {
				...
				std::cerr << SD_ERR << "[VP2 HTTP " << uuid << "] protocol: "
						  << "invalid size " << size
						  << std::endl;
			}
			...
			bool ret = true;
			date::sys_seconds lastArchive = date::floor<std::chrono::seconds>(std::chrono::system_clock::from_time_t(lastDownload));
			...
			const VantagePro2ArchiveMessage::ArchiveDataPoint* pastLastDataPoint = dataPoint + (size / sizeof(VantagePro2ArchiveMessage::ArchiveDataPoint));
			...
			for ( ; dataPoint < pastLastDataPoint && ret ; ++dataPoint) {
				...
				if (message.looksValid()) {
					...
					std::cerr << SD_WARNING << "[VP2 HTTP " << uuid << "] measurement: "
							  << "record looks invalid for station " << name << ", discarding..."
							  << std::endl;
				}
			...
			if (ret) {
				std::cout << SD_DEBUG << "[VP2 HTTP " << uuid << "] measurement: "
						  << "archive data stored for station " << name
						  << std::endl;
				...
				if (!ret)
					std::cerr << SD_ERR << "[VP2 HTTP " << uuid << "] management: "
							  << "couldn't update last archive download time for station " << name
							  << std::endl;
			} else {
				std::cerr << SD_ERR << "[VP2 HTTP " << uuid << "] measurement: "
