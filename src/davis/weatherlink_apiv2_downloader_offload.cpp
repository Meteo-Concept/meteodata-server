/**
 * @file weatherlink_apiv2_downloader_offloader.cpp
 * @brief A downloader that stores real-time Weatherlink v2 content into a job queue
...
int main(int argc, char** argv)
{
	...
	if (vm.count("help")) {
		...
		std::cout << desc << "\n";
		std::cout << "You must give either both the username and password or none of them." << std::endl;
		...
		for (auto it = weatherlinkStations.cbegin() ; it != weatherlinkStations.cend() ; ++it) {
			...
			try {
				if (!std::get<1>(station)) {
					WeatherlinkApiv2Downloader::downloadOnlyRealTime(db, client, std::get<0>(station), std::get<3>(station), weatherlinkApiV2Key, weatherlinkApiV2Secret);
				} else {
					std::cerr << "Station " << std::get<0>(station) << " has access to archives, ignoring" << std::endl;
				}
