...
{
	static const std::map<uint8_t, std::string> forecasts = {{0,   "Mostly clear and cooler."},
...
{
	static const std::map<uint8_t, std::string> bartrends = {{196, "Falling rapidly"},
															 {236, "Falling slowly"},
															 {0,   "Steady"},
															 {20,  "Raising slowly"},
															 {60,  "Raising rapidly"}};
...
{
	return validateCRC(&_l1, sizeof(Loop1)) && validateCRC(&_l2, sizeof(Loop2));
}
}
